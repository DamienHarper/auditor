...
> [!TIP]
> PHP attributes have been promoted to the core `DH\Auditor\Attribute` namespace to be provider-agnostic (e.g. for future Eloquent provider support). The old provider-specific classes are now **deprecated** but still work as they extend the new ones.
...
The provider runs first; the listener can override or extend its output. See the [Extra Data guide](../extra-data.md) for precedence rules, merging strategies, and caveats.
